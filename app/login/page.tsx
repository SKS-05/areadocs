  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        router.push(`/reset-password?uid=${user.uid}`);
        return;
      }

      if (userDoc.data().passwordresetrequired) {
        // Redirect to Reset Password Page if reset is required
        router.push(`/reset-password?uid=${user.uid}`);
      } else {
        // Redirect to homepage if no reset is required
        router.push("/");
      }
    } catch (err: any) {
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-2 mt-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
